...
class QWenMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        up_proj, gate_proj = self.upgate_proj(hidden_states.to(torch.float32))
        return self.c_proj(up_proj*F.silu(gate_proj)).to(hidden_states.dtype)
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None and torch.any(input_ids == self.config.visual['image_start_id']):
            ...
            img_pos = torch.stack((bos_pos[0], bos_pos[1], eos_pos[1]), dim=1) # 明确图片在tokens中的位置
            images = []
        ...
        # print("="*20,input_ids,inputs_embeds)
        ...
        if fake_images is not None:
            ...
            for idx, (i, a, b) in enumerate(img_pos):
                hidden_states[i][a + 1 : b] = images[idx] # 将图片的嵌入放入到文本中的图片区域，也就是在这一步，将图片特征与文本特征相融合。
