...
class E8P12_codebook(nn.Module):
    ...
    def round(self, X, grid, grid_norm):
        ...
        Xqidx = (2 * X @ grid.T - grid_norm).argmax(-1) # 计算与E8网格中距离最近的点位
        return grid[Xqidx], Xqidx

    def fast_quantize_part(self, X, parity):
        X_part = torch.abs(X)
        X_odd = torch.where((X < 0).sum(dim=-1) % 2 != 0)[0] # 小于0的是奇数个的索引
        X_part[X_odd, 7] = -X_part[X_odd, 7]
        mask = 1 - 2 * (X < 0).to(torch.float32) # 小于0的是-1，大于0的是1
        mask[X_odd, 7] = -mask[X_odd, 7]
        roundout, Xqidx = self.round(X_part, self.grid_part,
                                     self.grid_part_norm) # roundout是距离X中向量最近的网格中的点的值，Xqidx是索引
        vals = roundout * mask # 还原正负性
        err = (X - vals).norm(dim=-1) # 计算与X的距离
        abs_idx = self.part_abs_map[Xqidx] # 映射到E8码表上了
        sign_mask = (((roundout < 0) ^ (mask < 0))[:,
                                                   [0, 2, 4, 6, 1, 3, 5, 7]])
        sign_mask[:, 7] = sign_mask[:, 7] ^ self.grid_abs_odd[abs_idx]
        sign_mask[:, 0] = sign_mask[:, 0] ^ parity # parity决定符号时候翻转，为True时翻转符号
