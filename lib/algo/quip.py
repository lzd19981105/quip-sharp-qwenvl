...
def quantize_linear(weights, save_path, hessian_path, cb, args, device='cpu',bias=None):
    dtype_ = torch.float64 if args.use_fp64 else torch.float32
    ...
    if len(scales) == 1:
        scales[0] = scales[0].to(attr["SV"].device)
    ...
    utils.show_metrics(hatW, W.to("cpu"), H.to(dtype_), save_path)
    utils.clean()
